...
const Navbar:React.FC<NavbarProps> = () => {
    const setAutoModelState = useSetRecoilState(authModelState)
    const handleClick =()=>{
        setAutoModelState((prev) => ({...prev,isOpen: true}));

    }
    
    return <div className="flex items-center justify-between sm:px-12 px-2 md:px-24 ">
        <Link href ="/" className="flex items-center justify-center h-20">
            <Image src ="/logo.png" alt="leetclone" height={200} width={200} className='h-full'/>
        </Link>
